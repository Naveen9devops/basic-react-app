pipeline {
  ...
  parameters {
    string(name: 'IMAGE_REPO_NAME', defaultValue: 'naveenanthati3/basic-react', description: '')
  ...
  stages {
    ...
    stage('docker build'){
      ...
      steps{
        ...
        sh "docker tag $BUILD_IMAGE_REPO_TAG ${params.IMAGE_REPO_NAME}:${readJSON(file: 'package.json').version} || true"
